...
sir.sid <- function(sID,selex=FALSE,ncores=1)
{
	with(sID,{
		...
		sID$nll_noLC    <- plyr::ldply(cmsy,function(x){c("nll_noLC"=x[['nll_noLC']])})
		sID$like_lc <- plyr::ldply(cmsy,function(x){c("like_lc"=x[['like_lc']])})
		...
		sID$wts    <- exp(-(sID$nll[-3] + sID$prior))
